...
function getCategories(request, response) {
  ...
  const sqlOpdracht = db.prepare('SELECT * FROM products ORDER BY product.name')
...
function getProducts(request, response) {
  ...
  let data = []
  if (category_id > 0) {
    const sqlOpdracht = db.prepare('SELECT * FROM products WHERE category_id = ? ORDER BY product.name')
